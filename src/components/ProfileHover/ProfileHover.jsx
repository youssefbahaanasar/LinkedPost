...
export default function ProfileHover({name,photo,username,userId}) {
  ...
  return <>
    <HoverCard>
        <HoverCardTrigger onClick={()=>navigate(`/profile/${userId}`)} className={`${username?'text-md':'text-xs'} capitalize font-semibold cursor-pointer line-clamp-1 hover:underline`}>{name}</HoverCardTrigger>
        ...
        <div className="flex justify-around flex-col gap-2 pt-1">
            <div className="">
            <h2  onClick={()=>navigate(`/profile/${userId}`)} className="text-lg cursor-pointer capitalize hover:underline line-clamp-1" >{name.split(' ',2).join(' ')}</h2>
